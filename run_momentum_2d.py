...
def getArguments():
    ...
    parser.add_argument('--momentum', type=float, default=0.3,
                        help='Momentum for gradient descent. Defaults to 0.3')
...
def plotAndSaveGraphs(gd, args):
    ...
    # save animation
    if args.save:
        fileName = os.path.join(pathToDirectory, 'GradientDescentMomentum2D.mp4')
    ...
    if args.save:
        fileName = os.path.join(pathToDirectory, 'DistributionOfGradientsMomentum2D.png')
