...
for epoch in range(max_epoch):
    # gradient descent per mini batch
    ...
    # compute Log loss
    cost = ((1 / m) * np.sum(_y * np.log(hypothesis) + (1 - _y) * np.log(1 - hypothesis))) * -1
    # calculate gradients
    gradients = np.sum(((hypothesis - _y) * _x), keepdims=True, axis=0).T
    # update theta
    ...
    if not epoch % 50 or epoch == max_epoch - 1:
